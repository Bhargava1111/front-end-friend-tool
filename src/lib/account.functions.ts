...
function normalizeVerificationPayload(data: VerificationPayload) {
...
export async function getMyVerificationClient() {
  const token = await ensureValidAccessToken();
  if (!token) throw new Error("Please sign in to continue.");
  return apiFetch<AccountVerification>("/account/verification/", { token });
}

export async function submitVerificationClient(opts: { data: VerificationPayload }) {
  const token = await ensureValidAccessToken();
  if (!token) throw new Error("Please sign in to continue.");
  const payload = normalizeVerificationPayload(opts.data);
  return apiFetch<AccountVerification>("/account/verification/", {
    method: "POST",
    token,
...
export const submitVerification = createServerFn({ method: "POST" })
  .middleware([requireAuth])
  .inputValidator((data: VerificationPayload) => data)
  .handler(async ({ data, context }) => {
    return apiFetch<AccountVerification>("/account/verification/", {
      method: "POST",
      token: context.accessToken,
      body: toJsonBody(normalizeVerificationPayload(data)),
    });
  });
